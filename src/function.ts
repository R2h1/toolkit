...
const isAsyncFunction = (val: unknown) => Object.prototype.toString.call(val) === '[object AsyncFunction]';
...
const isGeneratorFunction = (val: unknown) => Object.prototype.toString.call(val) === '[object GeneratorFunction]';
 ...
 */
const boxHandler = <T>(x: T) => ({
  next: (f: (val: T) => any) => boxHandler(f(x)),
  done: (f: (val: T) => any) => f(x)
});
 ...
 */
const pipe = <T>(...fns: ((val: T) => T)[]) => (x: T) => fns.reduce((y, f) => f(y), x);
 ...
 *   fn('Hello World') === 'dlrow olleH';
 */
const compose = <T>(...fns: ((val: T) => T)[]) => (x: T) => fns.reduceRight((y, f) => f(y), x);
 ...
 */
const unary = <T>(fn: (val: T, ...arg: any[]) => any) => (arg: T) => fn(arg);
 ...
 *   curry(sum, 1, 2, 3); // 6
 */
const curry = (fn: Function, ...args: any[]): any => {
 ...
 */
const thunkfy = (fn: (...args: any[]) => any) => (...args: any[]) => () => fn(...args);
 ...
 */
const flip = (fn: (...args: any[]) => any) => (...args: any[]) => fn(...args.reverse());
 ...
 *   inc(9); // 10
 */
const partial = (fn: (...args: any[]) => any, ...a: any[]) => (...b: any[]) => fn(...a, ...b);
 ...
 */
const uncurry = (fn: Function, n = 1) => (...args: any[]) => {
  const outArgs = args.slice(0, n);
  return (
    (acc) => (innerArgs: any[]) => innerArgs.reduce((x: (arg0: any) => any, y: any) => x(y), acc)
  )(fn)(outArgs);
};
